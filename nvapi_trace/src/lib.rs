...
impl GlobalState {
    ...
    fn get_thunk(
        &mut self,
        original: *mut c_void,
        report_fn: unsafe extern "C" fn(i32, u32),
        interface: u32,
    ) -> *mut c_void {
...
macro_rules! interface_to_name {
    ($({ $name:literal, $id:literal }),* $(,)?) => {
        fn interface_to_name(interface: u32) -> Option<&'static str> {
            match interface {
                $(
                    $id => Some($name),
                )*
...
pub unsafe extern "C" fn nvapi_QueryInterface(interface: u32) -> *mut c_void {
    ...
    let format_args = dark_api::FnFfiWrapper(|| {
        ...
        let name = interface_to_name(interface);
        match name {
            Some(name) => write!(&mut writer, "(interface: {})", name).ok(),
            None => write!(&mut writer, "(interface: {:#010x})", interface).ok(),
        };
...
unsafe extern "C" fn report_fn(result: i32, interface: u32) {
...
pub fn get_thunk(
    ...
    _report_fn: unsafe extern "C" fn(i32, u32),
    _interface: u32,
) -> *mut c_void {
    todo!()
}
