...
fn escape(content: &[u8]) -> Vec<u8> {
    ...
    result.extend_from_slice("eptx0001".as_bytes());
...
fn encode(content: &mut [u8], reduced_key: &GenericArray<u8, U16>) {
    for i in 0..content.len() / 8 {
        let block = &mut content[i * 8..i * 8 + 8];
        unsafe { encrypt_block(block.try_into().unwrap(), reduced_key) };
    }
}

unsafe fn encrypt_block(block: &mut [u8; 8], key: &GenericArray<u8, U16>) {
    let delta: u32 = 0x9E3779B9;
    let mut sum: u32 = 0;
    let v = mem::transmute::<[u8; 8], [u32; 2]>(*block);
    let mut v0 = v[0];
